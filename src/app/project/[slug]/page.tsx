...
export default async function ProjectDetail({ params }: Props) {
  ...
  return (
    <>
      <main className="bg-white min-h-screen">
        ...
        <section className="relative pt-32 pb-16 lg:pt-48 lg:pb-32 overflow-hidden">
          ...
          <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10">
            <div className="absolute -top-[20%] -right-[10%] w-[70%] h-[70%] rounded-full bg-gradient-to-b from-blue-50 to-transparent opacity-70 blur-[100px]"></div>
            <div className="absolute top-[40%] -left-[10%] w-[50%] h-[50%] rounded-full bg-gradient-to-t from-amber-50 to-transparent opacity-60 blur-[100px]"></div>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-center">
              ...
              <div className="lg:col-span-5 text-left order-2 lg:order-1">
                {project.is_promo && (
                  <div className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-100 to-orange-100 text-amber-900 text-xs font-bold px-4 py-1.5 rounded-full mb-6 uppercase tracking-widest shadow-sm ring-1 ring-amber-200/50">
                    <span className="animate-pulse">🔥</span> Promo Eksklusif
                  </div>
                )}
                
                <h1 className="text-4xl md:text-5xl lg:text-[3.5rem] font-extrabold text-gray-900 mb-6 tracking-tight leading-[1.1]">
                  {project.name}
                </h1>
                
                {project.location && (
                  <div className="flex items-center gap-3 text-gray-600 text-lg font-medium mb-8">
                    <div className="flex items-center justify-center w-10 h-10 bg-gray-50 border border-gray-100 rounded-full shadow-sm">
                      <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a2 2 0 01-2.828 0l-4.244-4.243a8 8 0 1111.314 0z"/><circle cx="12" cy="11" r="3" strokeWidth="2"/></svg>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <a href={projectWaLink} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center justify-center gap-3 px-8 py-4 rounded-2xl bg-gray-900 text-white font-semibold text-lg hover:bg-gray-800 hover:shadow-2xl hover:shadow-gray-900/20 transition-all duration-300 transform hover:-translate-y-1">
                    <svg className="w-6 h-6 text-green-400 group-hover:scale-110 transition-transform duration-300" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.71.306 1.263.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.247-.694.247-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                  Tanya Detail & Harga
                </a>
              </div>
            </div>
      ...
      <section className="bg-gray-50/50 border-t border-gray-100 py-20 lg:py-32">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-16 lg:mb-24">
                  <h2 className="text-3xl lg:text-4xl font-extrabold text-gray-900 tracking-tight mb-4">Katalog Digital</h2>
              ...
              <div className="space-y-20 lg:space-y-32">
                  {project.project_images.map((image, index) => (
                      <div className="flex flex-col group" key={image.id}>
                          <div className="w-full overflow-hidden rounded-[1.5rem] lg:rounded-[2rem] bg-white border border-gray-100 shadow-xl shadow-gray-200/40 transition-transform duration-500 hover:-translate-y-2">
                              <img 
                          ...
                          {image.caption && (
                              <div className="mt-6 px-2 lg:px-4">
                                <p className="text-gray-900 text-xl md:text-2xl font-semibold tracking-tight">
                                  {image.caption}
                                </p>
                              </div>
                          )}
