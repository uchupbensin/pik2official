                ...
                <div className="max-w-[100rem] mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-16 sm:h-24">
                        ...
                        <Link href="/" className="flex items-center gap-3 group z-50">
                            {/* Jika logo berupa gambar: bisa ditukar antara putih (saat transparan) dan warna saat scrolled. Karena tidak ada dua logo, gunakan teks. */}
                            {isTransparent ? (
                                <span className="font-extrabold tracking-tight text-xl sm:text-2xl whitespace-nowrap drop-shadow-sm flex items-center gap-2 text-white">
                                    {siteName}
                                </span>
                            ) : (
                        ...
                        {/* Desktop nav */}
                        <div className="hidden lg:flex lg:items-center lg:justify-center lg:gap-4 absolute left-1/2 -translate-x-1/2">
                            {menus.map((menu) => {
                                ...
                                return (
                                    <div className="relative group" key={menu.label}>
                                        <Link href={menu.url}
                                            ...
                                            className={`flex items-center gap-1.5 px-4 py-2 text-[15px] font-medium transition-all duration-300 rounded-full ${isTransparent ? 'text-white/90 hover:text-white hover:bg-white/10' : 'text-gray-600 hover:text-[#1E356A] hover:bg-gray-50'}`}>
                                            {menu.label}
                                            {hasChildren && (
                                                <svg className={`w-3.5 h-3.5 transition-transform duration-300 group-hover:rotate-180 ${isTransparent ? 'text-white/70' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
                                            )}
                                        </Link>

                                        {/* Dropdown */}
                                        {hasChildren && (
                                            <div className="absolute left-1/2 -translate-x-1/2 top-full pt-4 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                                                <div className="bg-white text-gray-800 border border-gray-100 shadow-xl rounded-2xl overflow-hidden py-2 relative">
                                                    ...
                                                    {menu.children!.map((child) => (
                                                        <Link href={child.url}
                                                            ...
                                                            className="block px-5 py-3 text-sm font-medium text-gray-600 hover:text-[#1E356A] hover:bg-gray-50/80 transition-colors">
                                                            {child.label}
                                                        </Link>
                                                    ))}
                                                </div>
                        ...
                        <div className="flex items-center gap-4 z-50">
                            ...
                            <div className="hidden lg:block">
                                <a href={waLink} target="_blank" rel="noopener noreferrer"
                                    className={`group inline-flex items-center gap-2 px-6 py-2.5 rounded-full text-sm font-bold transition-all duration-300 ${isTransparent ? 'bg-white text-[#1E356A] hover:shadow-lg hover:-translate-y-0.5' : 'bg-[#81A649] text-white hover:shadow-[0_8px_20px_rgba(129,166,73,0.3)] hover:-translate-y-0.5'}`}>
                            ...
                            {/* Mobile hamburger */}
                            <button type="button" className={`lg:hidden inline-flex items-center justify-center p-2 rounded-xl transition-colors ${isTransparent ? 'text-white hover:bg-white/10' : 'text-gray-800 hover:bg-gray-100'}`}
